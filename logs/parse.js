...
fs.readFileSync(filename).toString().split('\n').forEach(function(line) {
  match = line.match(/Starting at (\d+)\.\.\./)
  if (match) results.push(nextBlock = { start: parseInt(match[1]), clicks: [] })
  match = line.match(/(\d+) (\d+)x(\d+)/)
  if (match) nextBlock.clicks.push({ uts: match[1], x: match[2], y: match[3] }) // TODO: Parse full id, don't break to x,y
})
